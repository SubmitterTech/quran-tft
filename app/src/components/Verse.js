    ...
    return (
        <div className={`relative`}>
            <div className={`absolute w-full flex h-full justify-between px-2.5`}>
                <div className={`flex h-full w-full px-2 justify-start items-start ${bookmark ? `${colors[theme]["accent"]["top"]}` : `${colors[theme]["text"]["on-map"]}`}`}>
                    {bookmark ?
                ...
                <div className={`flex h-full w-full justify-end items-start px-2 ${Math.abs(swipeDistance) > 100 ? `${colors[theme]["accent"]["top"]}` : `${colors[theme]["text"]["on-deep"]}`}`}>
