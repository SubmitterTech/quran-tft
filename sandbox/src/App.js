...
function App() {
  ...
  const countWordOccurrences = () => {
    ...
    Object.values(quranMap).forEach(verses => {
      Object.values(verses).forEach(verse => {
        count += verse.split(" ").filter(word => word.trim() === selectedWord).length;
      });
    });
    return count;
  };
  ...
  return (
    <div className="App w-screen h-screen bg-neutral-500 text-neutral-100 flex ">
      <div className="flex flex-col w-full overflow-auto h-full">
        {selectedWord && (
          <div className="rounded shadow-lg p-2 text-start m-2 bg-purple-700">
            Filter: {selectedWord} (Occurrences: {countWordOccurrences()})
          </div>
        )}

        {Object.entries(quranMap).map(([sura, verses]) => (
          <div key={sura} className="flex flex-col space-y-1 m-2">
            ...
            {Object.entries(filterVersesByWord(verses)).map(([vno, verseText]) => (
              <div key={vno} className="text-start w-full flex justify-between space-x-3">
                <div className="w-16 p-2 rounded shadow-lg bg-sky-600 text-end">{sura}:{vno}</div>
                <div
      ...
      <div className="flex flex-col w-full overflow-auto h-full ">
        <div className="w-full p-1">
          ...
          <div className="p-2 bg-orange-700 mb-4 mt-1 rounded shadow-lg">
